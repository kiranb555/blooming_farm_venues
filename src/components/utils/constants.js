export const FORMATED_PHONE = '800-400-8667';
export const PHONE = '8004008667';
export const EMAIL = 'bloomingfarmsvenue@gmail.com';
export const ADDRESS = '2940 Jim Johnson Rd, Concord, NC 28027, USA';
export const SOCIAL_MEDIA = {
    INSTAGRAM: 'https://www.instagram.com/bloomingfarmsvenue/',
    FACEBOOK: 'https://www.facebook.com/people/Blooming-Farms-Venue/61573805087553/',
    YOUTUBE: 'https://www.youtube.com/@BloomingFarmsVenue',
    TWITTER: 'https://twitter.com/bloomingfarmsvenue',
    LINKEDIN: 'https://www.linkedin.com/company/bloomingfarmsvenue/',
    WHATSAPP: `https://wa.me/1${PHONE}?text=Hello%20Bloom%20Farm%20Venues!`
};  

export const LATTITUDE = 35.4538863;
export const LONGITUDE = -80.7449327;
